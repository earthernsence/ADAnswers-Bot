...
export const ImaginaryUpgrades: IImaginaryUpgrades = {
  ...
  suspicionOfInterference: new ImaginaryUpgrade({
    ...
    strategy:
      "This one, you'll probably need Nameless 25 (for the extra Time glyph effect), and a max Reality Glyph. It may take some time to grind the Relic Shards up to 1e90, but it shouldn't be too long."
  }),
  ...
  recollectionOfIntrusion: new ImaginaryUpgrade({
    ...
    strategy:
      "Hint: ||Time Shards are especially helpful, wait a bit to let them build up before entering EC5||\nSolution: ||ytttt/yettt||"
  }),
  fabricationOfIdeals: new ImaginaryUpgrade({
    ...
    strategy:
      "Hint: ||remember to disable the two ways you gain IDs: turn off auto-EC and ID autobuyers, then Reality again. If necessary, you can enter EC2, then re-enable auto EC and ID autobuyers. This will need completed Ra and a good amount of other stats!||\nSolution: ||yettt||"
  }),
  ...
  dimensionalSymmetry: new ImaginaryUpgrade({
    ...
    strategy:
      "Hint: ||you'll need around 1.5e20 iM, e74-e75 Glyph Sacrifice, 50+% Lai'tela continuum bonus, and near e200 years||\nSolution: ||ydddd||"
  }),
  deterministicRadiation: new ImaginaryUpgrade({
    ...
    strategy:
      "Hint: ||Make sure to do something you haven't done in a long time.||\nSolution: ||iiiep + `11,21,31,41,51,61,72,82` + Crunch + auto-ECs||"
  }),
  ...
  planarPurification: new ImaginaryUpgrade({
    ...
    strategy:
      "Hint: ||at most 0 Glyphs equipped...? What decreases the number of Glyphs equipped?||\nSolution: ||cyer (cursed is -3)||"
  }),
  ...
  omnipresentObliteration: new ImaginaryUpgrade({
    ...
    strategy:
      "Hint: ||no way to get around the Glyphs equipped this time. What Glyph has given you progress in Lai'tela this whole time?||\nSolution: ||d, you'll need to wait for e4k EP (game speed increases as time goes by)||"
  })
};
